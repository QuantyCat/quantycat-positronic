...
Run with the openpi venv:
    cd vendor/openpi
    uv run python ../../eval/openpi/openpi_lora_high_motion_eval.py \
        --checkpoint ~/quantycat-data/checkpoints/openpi/pi05_quantycat_lora/<run>/<step>
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=DATA_HOME / "eval_output/openpi/pi05")
...
def _load_episodes(dataset_root: Path, episode_filter: list[int] | None = None) -> list[EpisodeData]:
    ...
    for parquet in sorted(
        (dataset_root / "data").glob("chunk-*/episode_*.parquet"), key=_episode_index
    ):
...
def _load_policy(args: argparse.Namespace):
    ...
    os.environ.setdefault("HF_LEROBOT_HOME", str(DATA_HOME / "datasets"))
...
def _eval_joint(
    ...
    for case_index, window in enumerate(windows, start=1):
        ...
        summary["action_convention"] = (
            "openpi LoRA commanded-delta eval: joints 0-4 compare policy target-current "
...
def main() -> int:
    ...
    if args.label is None:
        ...
        step = args.checkpoint.name
        args.label = f"openpi_lora_{run_name}_{step}"
