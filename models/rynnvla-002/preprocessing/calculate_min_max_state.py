...
"""
Analyzes robot State data in a given directory.

Recursively finds folders containing 'episode' subdirectories, then finds
all 'state/*.npy' files within them. Loads them in parallel using
multiple threads, validates shape is (6,), and prints statistics
(min, max, 1st percentile, 99th percentile) across all valid state data.

Usage:
    python calculate_min_max_state.py /path/to/training_data

Example:
    python calculate_min_max_state.py my_data/training_pipeline/training_data

This file was generated from RynnVLA-002/rynnvla-002/data_lerobot/calculate_min_max_all_data_state.py
"""
...
def find_all_state_npy_files_fast(root_directories):
    """
    Uses glob patterns to quickly find all state .npy files in the given directories.
    Assumes structure: <root_directory>/<any_name>/state/*.npy
    """
    npy_file_paths = []
...
def load_and_validate_state(file_path):
    """
    Worker function for a single thread: loads a .npy file and validates its shape.
    Returns the state data or None on failure.
    """
    try:
        state_data = np.load(file_path)
        if state_data.shape == (6,):
            return state_data
        else:
            print(f"\nWarning: file '{file_path}' has shape {state_data.shape}, expected (6,). Skipping.", file=sys.stderr)
            return None
    except Exception as e:
        print(f"\nError: could not load '{file_path}': {e}", file=sys.stderr)
        return None

def analyze_state_data_multithreaded(file_paths):
    """
    ...
    print(f"Loading and processing state data using up to {MAX_WORKERS} threads...")

    # Use ThreadPoolExecutor to load files in parallel.
    # executor.map passes each element of file_paths to load_and_validate_state.
    # We wrap the iterator with tqdm to show a progress bar.
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results_iterator = executor.map(load_and_validate_state, file_paths)
        all_states = [r for r in tqdm(results_iterator, total=len(file_paths), desc="Loading .npy files", unit="file") if r is not None]

    if not all_states:
        print("\nNo valid data loaded. Check file contents and shapes.")
        return

    # Stack all states into a single (N, 6) NumPy array
    print(f"\nData loaded. Stacking arrays...")
    stacked_states = np.array(all_states)

    print(f"Successfully loaded and stacked {stacked_states.shape[0]} states, shape: {stacked_states.shape}")
    print("Computing statistics...")

    min_vals = np.min(stacked_states, axis=0)
    max_vals = np.max(stacked_states, axis=0)
    q01_vals = np.percentile(stacked_states, 1, axis=0)
    q99_vals = np.percentile(stacked_states, 99, axis=0)
...
def run_analysis_on_dirs(input_dirs):
    ...
    all_files = find_all_state_npy_files_fast(all_episode_dirs)
    analyze_state_data_multithreaded(all_files)

def main():
    parser = argparse.ArgumentParser(
        description="Analyze robot state .npy files and compute statistics.",
