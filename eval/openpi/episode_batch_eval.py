...
_DEFAULT_CONFIG = "models/openpi/training_config/config.yaml"
...
def _action_bounds(norm_stats_json: "Path | str | None" = None) -> tuple[np.ndarray, np.ndarray]:
    path = Path(norm_stats_json) if norm_stats_json else Path(os.environ.get("OPENPI_NORM_STATS", ""))
    if not path or not path.is_file():
        raise FileNotFoundError(
            "No norm_stats.json found. Pass a path or set OPENPI_NORM_STATS env var.\n"
            "Example: ~/quantycat-data/norm_stats/openpi/<config_name>/norm_stats.json"
        )
    data = json.load(open(path, encoding="utf-8"))
    actions = data["norm_stats"]["actions"]
    return np.array(actions["q01"], dtype=np.float32), np.array(actions["q99"], dtype=np.float32)
...
def main() -> None:
    raise SystemExit(
        "episode_batch_eval.py no longer supports standalone execution.\n"
        "Use it as a library imported by openpi eval scripts."
    )
